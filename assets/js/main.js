(function ($) {
	...
	// --- Kedi Global Sync: Live Date & Cart/Wishlist ---
	function updateKediGlobalSync() {
		// 1. Live Date & Time
		const now = new Date();
		...
		$('.date').each(function() {
			$(this).html(`<i class="fal fa-calendar-alt"></i> ${dateString}`);
		});
		$('.time').each(function() {
			$(this).html(`<i class="fal fa-clock"></i> ${timeString}`);
		});

		// 2. Cart & Wishlist Sync (Standardized to kedi_cart)
		const cart = JSON.parse(localStorage.getItem('kedi_cart')) || [];
		const wishlist = JSON.parse(localStorage.getItem('wishlist')) || [];
		
		const cartTotalCount = cart.reduce((sum, item) => sum + (parseInt(item.quantity) || 1), 0);
		const wishlistTotalCount = wishlist.length;

		// Update icons in header
		$('.cart_btn .count, .cart-count, .header__icons .count, #cart-count').text(cartTotalCount);
		$('.wishlist-icon .count, .wishlist-count').text(wishlistTotalCount);

		// Update Cart Sidebar counts
		$('.cart_sidebar .heading_title span').text(cart.length);
	}

	// Initialize and run interval
	$(document).ready(function() {
		updateKediGlobalSync();
		setInterval(updateKediGlobalSync, 1000);
	});
	// --- End Kedi Global Sync ---
