...
const categoryImages = {
  Nutrition:
    'assets/img/gallery/Reishi.jpg',
  Wellness:
    'assets/img/gallery/testimony.jpg',
  Pharma:
    'assets/img/gallery/Cardibetter.jpg',
  'Mental Health':
    'assets/img/gallery/revive.jpg',
  BioTech:
    'assets/img/gallery/Roadmap.jpg',
  'Med Devices':
    'assets/img/gallery/Blood-circulatory-machine.png',
  'Public Health':
    'assets/img/gallery/Achievements.jpg',
  'Clinical Diagnostics':
    'assets/img/gallery/testimony.jpg',
  'Metabolic Health':
    'assets/img/gallery/Diawell.png',
  'Longevity':
    'assets/img/gallery/Golden six.png',
  'Vitality':
    'assets/img/gallery/Revive.png',
};
const authorPool = [
  'Dr. Aisha Rana',
  'Mina Patel',
...
const articleTemplates = {
  ...
  'Clinical Diagnostics': [
    'New AI Model Outperforms Radiologists in Early Breast Cancer Detection',
    'Rapid PCR Tests for Tropical Diseases Deployed to Rural Clinics',
    'Molecular Diagnostics Transformation: Scaling Precision Medicine',
  ],
  'Metabolic Health': [
    'Intermittent Fasting vs. Time-Restricted Feeding: New Clinical Meta-Analysis',
    'Insulin Sensitivity Breakthrough: The Role of Gut Microbiome Diversification',
    'Kedi Diawell Protocol Shows 30% Improvement in Glucose Regulation in Trials',
  ],
  Longevity: [
    'Senolytic Therapies: The Next Frontier in Age-Reversal Research',
    'Telomere Maintenance Through Specific Herbal Alkaloid Extraction',
    'Blue Zone Habits Integrated into Modern Urban Planning for Longevity',
  ],
  Vitality: [
    'Adrenal Fatigue Recovery: Rebalancing Cortisol Through Traditional Medicine',
    'Mitochondrial Optimization: How ATP Production Drives Mental Clarity',
    'Kedi Revive Protocol: Sustained Energy Without Stimulant Dependency',
  ],
};
...
const newsData = (typeof KediNews !== 'undefined') ? KediNews.getAll() : [...baseArticles, ...extraArticles];
const testimonials = [
  {
    quote:
      'Kedi Healthcare helped me discover the latest clinical trends without leaving the page. The reader view keeps me focused and engaged.',
        ...
        <article class="group rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-lg transition">
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="flex items-center justify-between gap-4 mt-2">
                <span class="text-lg font-black text-blue-600">0.0000${Math.floor(Math.random() * 9000) + 1000}π</span>
                <button onclick="KediCart.addToCart({id: '${item.slug}', name: '${item.title}', price: '0.0000${Math.floor(Math.random() * 9000) + 1000}π', img: '${item.img}'})" 
                        class="flex-1 rounded-full bg-slate-950 text-white py-3 text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition flex items-center justify-center gap-2">
                    <i class="fas fa-shopping-cart"></i> Add to Cart
                </button>
            </div>
            <button onclick="showProductUpsell('${item.slug}')" class="w-full text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-blue-600 transition mt-2">View Smart Pairings</button>
          </div>
        </article>
      `
    )
  ...
  const schema = [
    {
      ...
      description:
        'Kedi Healthcare is an AI-powered health news site featuring product intelligence, quiz navigation, and smart upsell recommendations.',
      potentialAction: [
        {
          '@type': 'SearchAction',
...
function openArticle(index) {
  ...
  // Use article ID from database if available, otherwise fallback to index
  const id = article.id || (index + 1);
  window.location.href = `news-single.html?id=${id}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initFromHash();
  initFromHash();
  // GCV calculation for Pi cryptocurrency
  const PI_PRICE = 314159;
  function calculateGCV(quantity = 1) {
    return quantity * PI_PRICE;
  }
  // expose globally
  window.calculateGCV = calculateGCV;
  // display on dashboard if element exists
  const gcvEl = document.getElementById('gcv-value');
  if (gcvEl) {
    gcvEl.innerText = '$' + calculateGCV().toLocaleString();
  }
});