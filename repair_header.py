"""
Repair script for corrupted header section in index.html.
Strategy: Read whole file as a string. Locate the two stable anchors:
  ANCHOR_START: end of the top bar (closing </div> for header__top)
  ANCHOR_END  : the stable nav bar wrapper opening tag
Replace everything between them with correct HTML.
"""
...
# ── Anchor 1 ─────────────────────────────────────────────────────────────────
# The top bar ends with the closing of header__top-info > ul (top-right) wrapper.
# The last reliably-unique string just before the broken zone is the currency
# switcher list-item closer.  We look for the very end of the top-bar container.
# The top-bar container pattern ends with a sequence: </div> </div> </div>
# matching: .aura-currency-switcher  </div> </li> </ul> </div> </div> </div>

ANCHOR_START_PATTERN = r'(data-currency-set="PI">[\s\S]*?</div>\s*</div>\s*</div>)'

# ── Anchor 2 ─────────────────────────────────────────────────────────────────
# The sticky nav bar wrapper that comes AFTER the header__middle section.
# This tag is stable and unique:
ANCHOR_END_STR = '<div class="header__wrap" data-uk-sticky='
...
# ── Find anchor 1: end of the top bar ────────────────────────────────────────
# The top bar closes right after the currency switcher.
# We search for the Pi currency entry and find the THREE closing divs that end:
#   </div>   ← .aura-currency-switcher
#   </div>   ← .header__top (or .container)
#   </div>   ← outermost top-bar wrapper
ANCHOR_START_RE = re.compile(
    r'(data-currency-set="PI">[^<]*</a>\s*</li>\s*</ul>\s*</li>\s*</ul>\s*</div>\s*</li>\s*</ul>\s*</div>\s*</div>\s*</div>)',
    re.DOTALL
)

m_start = ANCHOR_START_RE.search(content)
if not m_start:
    print("ERROR: Could not find ANCHOR_START (currency PI closer). Aborting.")
    sys.exit(1)

anchor_start_end = m_start.end()
print(f"ANCHOR_START found at char {m_start.start()} – {anchor_start_end}")
print(f"  Context: {repr(content[anchor_start_end-50:anchor_start_end+50])}")

# ── Find anchor 2: start of the header__wrap nav sticky bar ───────────────────
anchor2_idx = content.find(ANCHOR_END_STR, anchor_start_end)
if anchor2_idx == -1:
    print("ERROR: Could not find ANCHOR_END (header__wrap). Aborting.")
    sys.exit(1)
print(f"ANCHOR_END found at char {anchor2_idx}")
print(f"  Context: {repr(content[anchor2_idx-50:anchor2_idx+50])}")

# ── Stitch ────────────────────────────────────────────────────────────────────
before = content[:anchor_start_end]
after  = content[anchor2_idx:]
new_content = before + MIDDLE + after

with open(FILE, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"\nSUCCESS: File written ({len(new_content)} chars).")
print("Verify header__middle is present:", 'header__middle' in new_content)
print("Verify hacTrack carousel is present:", 'hacTrack' in new_content)
print("Verify header__wrap still present:", 'header__wrap' in new_content)
